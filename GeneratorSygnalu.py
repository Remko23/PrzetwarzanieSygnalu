...
class GeneratorSygnalu:
    def __init__(self, zrodlo_sygnalu=None, fabryka_generatorow=None, czestotliwosc_probkowania=1000.0, **kwargs):
        ...
        self.metody = {
            ...
            "s13": self._s13_zlozony_1,
            "s14": self._s14_zlozony_2,
            "s15": self._s15_zlozony_3,
        }
    ...
    def _s13_zlozony_1(self, czas_poczatkowy=0.0, czas_trwania=1.0, **_):
        for t in self._pobierz_czas():
            if czas_poczatkowy <= t <= czas_poczatkowy + czas_trwania:
                yield 2 * np.sin((2 * np.pi / 2.0) * t + np.pi / 2) + 5 * np.sin((2 * np.pi / 0.5) * t + np.pi / 2)
            else:
                yield 0.0

    def _s14_zlozony_2(self, czas_poczatkowy=0.0, czas_trwania=1.0, **_):
        for t in self._pobierz_czas():
            if czas_poczatkowy <= t <= czas_poczatkowy + czas_trwania:
                yield 2 * np.sin((2 * np.pi / 2.0) * t) + np.sin((2 * np.pi / 1.0) * t) + 5 * np.sin((2 * np.pi / 0.5) * t)
            else:
                yield 0.0

    def _s15_zlozony_3(self, czas_poczatkowy=0.0, czas_trwania=1.0, **_):
        for t in self._pobierz_czas():
            if czas_poczatkowy <= t <= czas_poczatkowy + czas_trwania:
                yield 5 * np.sin((2 * np.pi / 2.0) * t) + np.sin((2 * np.pi / 0.25) * t)
            else:
                yield 0.0

    @classmethod
    def z_tablicy(cls, probki, czestotliwosc_probkowania):
        def generator():
            for p in probki:
                yield p
            while True:
        ...
        return cls(fabryka_generatorow=generator, czestotliwosc_probkowania=czestotliwosc_probkowania)
